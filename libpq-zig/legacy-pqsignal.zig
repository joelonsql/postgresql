...
pub export fn pqsignal(arg_signo: c_int, arg_func: pqsigfunc) pqsigfunc {
    var signo = arg_signo;
    _ = &signo;
    var func = arg_func;
    _ = &func;
    ...
    act.sa_flags = 0;
    if (signo != @as(c_int, 14)) {
        act.sa_flags |= @as(c_int, 2);
    }
    if (signo == @as(c_int, 20)) {
        act.sa_flags |= @as(c_int, 8);
    }
    if (sigaction(signo, &act, &oact) < @as(c_int, 0)) return @as(?*const fn (c_int) callconv(.C) void, @ptrFromInt(-@as(c_int, 1)));
