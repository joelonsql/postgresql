...
pub const SASL_COMPLETE: c_int = 0;
pub const SASL_FAILED: c_int = 1;
pub const SASL_CONTINUE: c_int = 2;
pub const SASL_ASYNC: c_int = 3;
pub const SASLStatus = c_uint;
pub const struct_pg_fe_sasl_mech = extern struct {
    init: ?*const fn (?*PGconn, [*c]const u8, [*c]const u8) callconv(.C) ?*anyopaque = @import("std").mem.zeroes(?*const fn (?*PGconn, [*c]const u8, [*c]const u8) callconv(.C) ?*anyopaque),
    exchange: ?*const fn (?*anyopaque, bool, [*c]u8, c_int, [*c][*c]u8, [*c]c_int) callconv(.C) SASLStatus = @import("std").mem.zeroes(?*const fn (?*anyopaque, bool, [*c]u8, c_int, [*c][*c]u8, [*c]c_int) callconv(.C) SASLStatus),
    channel_bound: ?*const fn (?*anyopaque) callconv(.C) bool = @import("std").mem.zeroes(?*const fn (?*anyopaque) callconv(.C) bool),
    free: ?*const fn (?*anyopaque) callconv(.C) void = @import("std").mem.zeroes(?*const fn (?*anyopaque) callconv(.C) void),
};
pub const pg_fe_sasl_mech = struct_pg_fe_sasl_mech;
