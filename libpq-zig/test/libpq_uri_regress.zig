...
pub export fn main(arg_argc: c_int, arg_argv: [*c][*c]u8) c_int {
    ...
    var argv = arg_argv;
    _ = &argv;
    var opts: [*c]PQconninfoOption = undefined;
    _ = &opts;
    var defs: [*c]PQconninfoOption = undefined;
    _ = &defs;
    var opt: [*c]PQconninfoOption = undefined;
    _ = &opt;
    var def: [*c]PQconninfoOption = undefined;
    _ = &def;
    var errmsg: [*c]u8 = null;
    _ = &errmsg;
    var local: bool = @as(c_int, 1) != 0;
    _ = &local;
    if (argc != @as(c_int, 2)) return 1;
    opts = PQconninfoParse(argv[@as(c_uint, @intCast(@as(c_int, 1)))], &errmsg);
    if (opts == @as([*c]PQconninfoOption, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        _ = pg_fprintf(__stderrp, "libpq_uri_regress: %s", errmsg);
        return 1;
    }
    defs = PQconndefaults();
    if (defs == @as([*c]PQconninfoOption, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        _ = pg_fprintf(__stderrp, "libpq_uri_regress: cannot fetch default options\n");
        return 1;
    }
    {
        _ = blk: {
        ...
        while (opt.*.keyword != null) : (_ = blk: {
            ...
            if (opt.*.val != @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
                if ((def.*.val == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (strcmp(opt.*.val, def.*.val) != @as(c_int, 0))) {
                    _ = pg_printf("%s='%s' ", opt.*.keyword, opt.*.val);
                }
                if ((@as(c_int, @bitCast(@as(c_uint, opt.*.val.*))) != 0) and ((strcmp(opt.*.keyword, "hostaddr") == @as(c_int, 0)) or ((strcmp(opt.*.keyword, "host") == @as(c_int, 0)) and (@as(c_int, @bitCast(@as(c_uint, opt.*.val.*))) != @as(c_int, '/'))))) {
                    local = @as(c_int, 0) != 0;
                }
            }
        }
...
pub const OID_MAX = @compileError("unable to translate macro: undefined identifier `UINT_MAX`"); // src/include/postgres_ext.h:38:9
