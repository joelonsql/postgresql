 ...
 * can also be used as a fallback for systems where shm_open is not
 * available or can't be used for some reason.  Of course, mapping a file
 * residing on an actual spinning disk is a fairly poor approximation for
 * shared memory because writeback may hurt performance substantially,
 * but there should be few systems where we must make do with such poor
 * tools.
 *
