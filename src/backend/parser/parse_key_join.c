 ...
 *
 *		Return true if a GROUP BY or DISTINCT key preserves one existing
 *		row-coverage fact.
 *
								...
								RangeTblEntry *rte, Relation rel)
{
	 ...
	 * If no children exist, hold ShareLock until transaction end so CREATE
	 * TABLE ... INHERITS cannot add the first child before proof dependency
	 * creation catches up.
	 */
	if (rte->relkind == RELKIND_RELATION && rte->inh)
	{
		LockRelationOid(rte->relid, ShareLock);
		if (has_subclass(rte->relid))
		{
			UnlockRelationOid(rte->relid, ShareLock);
			return;
		}
	...
	foreach_oid(indexoid, RelationGetIndexList(rel))
	{
		...
		for (int i = 0; i < index->indnkeyatts; i++)
		{
			...
			eqop = get_opfamily_member_for_cmptype(indexrel->rd_opfamily[i],
												   opcintype, opcintype,
												   COMPARE_EQ);
			if (!key_join_equality_operator_is_usable(eqop, opcintype,
													  &eqtype, &deps))
			{
				usable = false;
				break;
		...
		index_close(indexrel, keep_index_lock ? NoLock : ShareLock);
	}
	...
	foreach_node(KeyJoinFact, old, src->keyJoinFacts->facts)
	{
		...
		switch (old->kind)
		{
			...
			case KJF_UNIQUE:
				if (!preserve_unique)
				{
					add_inactive_projected(dst, old, attrmap, KJI_JOIN_FANOUT);
					continue;
				}
									 ...
									 Oid *eqTypeOid, List **dependencies)
{
	...
	righttype = operform->oprright;
	rettype = operform->oprresult;
	...
	if ((OidIsValid(expectedTypeOid) && lefttype != expectedTypeOid) ||
		rettype != BOOLOID)
	{
		ReleaseSysCache(optup);
		return false;
