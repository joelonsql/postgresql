	...
	else if (IsA(n, JoinExpr))
	{
		...
		else if (j->fkJoin)
		{
			...
			if (fkjn->fkdir == FKDIR_FROM)
			{
				...
				referencing_rel = other_rel;	/* Left table is the
												 * referencing table */
				referenced_rel = r_nsitem;	/* Right table is the referenced
											 * table */
				referencing_cols = fkjn->refCols;	/* Columns from
													 * referencing table */
				referenced_cols = fkjn->localCols;	/* Columns from referenced
													 * table */
			}
			else
			{
				...
				referenced_cols = fkjn->refCols;	/* Columns from referenced
													 * table */
				referencing_cols = fkjn->localCols; /* Columns from
													 * referencing table */
			}
