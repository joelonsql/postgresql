...
AlterRole(ParseState *pstate, AlterRoleStmt *stmt)
{
	...
	/*
	 * Handle FIDO2 credential management (ADD CREDENTIAL / DROP CREDENTIAL)
	 */
	if (daddcredential)
	{
...
/*
 * AddRoleCredential - Add a FIDO2 credential to a role
 *
...
AddRoleCredential(Oid roleid, const char *credname, const char *pubkey_str)
{
	...
	/* sign_count (starts at 0) */
	values[Anum_pg_role_pubkeys_sign_count - 1] = Int64GetDatum(0);
...
/*
 * DropRoleCredential - Drop a specific FIDO2 credential from a role
 */
static void
...
/*
 * DropRoleCredentialAll - Drop all FIDO2 credentials from a role
 */
static void
