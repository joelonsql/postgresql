 ...
 *	  backend that is still positioned at the old head or somewhere in the range
 *	  written, avoiding unnecessary wakeups for idle listeners that have nothing
 *	  to read. It also kicks backends that are way behind to make them advance
 *	  their pointers.
 *
 ...
 * backends are interested in notifies we send.  However, we also need
 * to make sure that all other listeners will eventually advance their
 * queue position pointers, allowing the global tail to advance.  To
 * achieve this, we use a combination of two mechanisms.  Listeners that
 * are not advancing their positions, and are stationary at the old queue
 * head or somewhere in the range we just wrote, can safely be direct
 * advanced to the new queue head, without signaling.  Listeners that
 * could not be direct advanced, and have fallen too far behind are
 * instead signalled, so that they'll advance their queue position
 * pointers themselves.
 *
