			...
			switch (current_state)
			{
				case ASYNC_STATE_IDLE:
					...
					if (expected == ASYNC_STATE_SIGNALLED)
					{
						...
						if (Trace_notify)
							elog(DEBUG1, "SignalBackends: backend %d already SIGNALLED by another notifier", pid);
						goto done_with_backend;
					}
					/* State is now PROCESSING - loop to handle it */
					break;
				...
				case ASYNC_STATE_PROCESSING:
					...
					if (expected == ASYNC_STATE_SIGNALLED)
					{
						/* Another notifier already requested re-scan - we're done */
						if (Trace_notify)
							elog(DEBUG1, "SignalBackends: backend %d already has re-scan requested by another notifier", pid);
						goto done_with_backend;
					}
					/* State is now IDLE - loop to handle it */
					break;
...
ProcessIncomingNotify(bool flush)
{
	...
	/*
	 * Loop to handle re-signaling. After we finish processing and try to
	 * return to IDLE, we might find that a notifier has set our state
	 * back to SIGNALLED, requiring us to process again.
	 */
	do
	{
		/*
		...
		else
		{
			...
			if (expected == ASYNC_STATE_SIGNALLED)
			{
				/* A notifier added more work - loop back to process it */
				process_more = true;
				if (Trace_notify)
					elog(DEBUG1, "ProcessIncomingNotify: notifier requested re-scan, looping");

				/* Transition back to PROCESSING for the next iteration */
				expected = ASYNC_STATE_SIGNALLED;
				if (!pg_atomic_compare_exchange_u32(&QUEUE_BACKEND_STATE(MyProcNumber),
												   &expected,
												   ASYNC_STATE_PROCESSING))
				{
					...
					elog(ERROR, "unexpected async state %u when re-entering PROCESSING",
						 expected);
				}
			}
			else
