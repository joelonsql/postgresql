...
-- Regression: recursive CTE validation must inspect a FOR KEY FILTER
-- before parse analysis merges it into the transformed join quals.
...
-- rejected, reason: v1 is filtered (referenced) but t2 (referencing) applies no
-- equivalent filter — the matched-filter rule cannot rescue this
...
-- rejected, reason: matching inequalities are not accepted by the conservative
-- matched-filter rule, even when the expression trees line up under the FK
-- column mapping.
SELECT * FROM v1 JOIN v2 FOR KEY (c3) -> v1 (c1);

-- rejected, reason: referenced-side HAVING filter is not a matched key filter
SELECT * FROM
(
...
(
    SELECT
        ...
        q2.c12
    FROM q2
-- rejected, reason: referenced-side q1 is filtered on a non-key column for c9,c10 coverage
...
-- rejected, reason: key join on subquery with GROUPING SETS
SELECT * FROM t1 JOIN
(
    SELECT c3, c4 FROM t2
...
-- rejected, reason: referenced-side filter is not matched on the referencing side
...
-- GROUP BY/DISTINCT do not preserve row coverage for nullable covered keys,
-- because row collapse treats nulls as equal even when UNIQUE treats them as
-- distinct.
...
-- rejected, reason: GROUP BY on nullable key columns cannot preserve row coverage
JOIN rowcov_nullable_child c FOR KEY (parent_code) -> p (code);

SELECT p.code, c.id AS child_id
FROM (SELECT DISTINCT code FROM rowcov_nullable_parent) p
-- rejected, reason: DISTINCT on nullable key columns cannot preserve row coverage
JOIN rowcov_nullable_child c FOR KEY (parent_code) -> p (code);
...
-- the exact key identity and not-null evidence.
...
-- rejected, reason: replay would require a new proof dependency
...
-- rejected, reason: replay would require child_b's NOT NULL proof
...
-- Stored key joins must be revalidated when NOT NULL inheritability changes.
CREATE TABLE alter_nn_parent
(
    id int PRIMARY KEY
);
CREATE TABLE alter_nn_child
(
    id int,
    parent_id int CONSTRAINT alter_nn_child_parent_id_nn NOT NULL,
    FOREIGN KEY (parent_id) REFERENCES alter_nn_parent (id)
) PARTITION BY RANGE (id);
CREATE TABLE alter_nn_child_0 PARTITION OF alter_nn_child
    FOR VALUES FROM (0) TO (100);
INSERT INTO alter_nn_parent VALUES (1);
INSERT INTO alter_nn_child VALUES (10, 1);

CREATE VIEW alter_nn_consumer AS
SELECT p.id AS parent_id, c.id AS child_id, c.parent_id AS child_parent_id
FROM alter_nn_parent p
-- accepted
JOIN alter_nn_child c FOR KEY (parent_id) -> p (id);

-- rejected, reason: stored view depends on an inherited NOT NULL proof
ALTER TABLE alter_nn_child
    ALTER CONSTRAINT alter_nn_child_parent_id_nn NO INHERIT;
SELECT * FROM alter_nn_consumer ORDER BY child_id;
DROP VIEW alter_nn_consumer;

ALTER TABLE alter_nn_child
    ALTER CONSTRAINT alter_nn_child_parent_id_nn NO INHERIT;
CREATE TABLE alter_nn_child_1 PARTITION OF alter_nn_child
    FOR VALUES FROM (100) TO (200);
INSERT INTO alter_nn_child_1 VALUES (110, NULL);
SELECT tableoid::regclass AS child_rel, id, parent_id
FROM alter_nn_child
ORDER BY id;
SELECT p.id AS parent_id, c.id AS child_id
FROM alter_nn_parent p
-- rejected, reason: inherited partitioned scan can now contain null keys
JOIN alter_nn_child c FOR KEY (parent_id) -> p (id);
SELECT c.id AS child_id, c.parent_id AS child_parent_id, p.id AS parent_id
FROM alter_nn_child c
-- accepted, reason: nullable referencing rows are preserved
LEFT JOIN alter_nn_parent p FOR KEY (id) <- c (parent_id)
ORDER BY child_id;
DROP TABLE alter_nn_child, alter_nn_parent;
...
END;
-- rejected, reason: replay would require the parent_b FK proof
...
CREATE ROLE kj_cross_owner_producer;
CREATE ROLE kj_cross_owner_consumer;
CREATE SCHEMA kj_cross_owner_p AUTHORIZATION kj_cross_owner_producer;
CREATE SCHEMA kj_cross_owner_c AUTHORIZATION kj_cross_owner_consumer;

SET ROLE kj_cross_owner_producer;
...
GRANT USAGE ON SCHEMA kj_cross_owner_p TO kj_cross_owner_consumer;
GRANT SELECT ON kj_cross_owner_p.parent_v TO kj_cross_owner_consumer;
GRANT SELECT ON kj_cross_owner_p.child TO kj_cross_owner_consumer;
RESET ROLE;

SET ROLE kj_cross_owner_consumer;
...
DROP ROLE kj_cross_owner_consumer;
DROP ROLE kj_cross_owner_producer;
...
-- RLS policy USING expressions store analyzed key-join proofs and must be
-- revalidated when their proof dependencies change.
CREATE TABLE policy_using_kj_parent (id int PRIMARY KEY);
CREATE TABLE policy_using_kj_child
(
    id int PRIMARY KEY,
    parent_id int NOT NULL REFERENCES policy_using_kj_parent (id)
);
CREATE TABLE policy_using_kj_guarded (id int);
INSERT INTO policy_using_kj_parent VALUES (1);
INSERT INTO policy_using_kj_child VALUES (10, 1);
ALTER TABLE policy_using_kj_guarded ENABLE ROW LEVEL SECURITY;
CREATE POLICY policy_using_kj ON policy_using_kj_guarded
USING (EXISTS (
    SELECT 1
    FROM policy_using_kj_parent p
    -- accepted
    JOIN policy_using_kj_child c FOR KEY (parent_id) -> p (id)
));

-- rejected, reason: stored policy USING expression depends on the FK proof
ALTER TABLE policy_using_kj_child
    ALTER CONSTRAINT policy_using_kj_child_parent_id_fkey NOT ENFORCED;

-- After dropping the policy, the same DDL succeeds.
DROP POLICY policy_using_kj ON policy_using_kj_guarded;
ALTER TABLE policy_using_kj_child
    ALTER CONSTRAINT policy_using_kj_child_parent_id_fkey NOT ENFORCED;
SELECT *
FROM policy_using_kj_parent p
-- rejected, reason: FK is now not enforced
JOIN policy_using_kj_child c FOR KEY (parent_id) -> p (id);
DROP TABLE policy_using_kj_guarded,
           policy_using_kj_child,
           policy_using_kj_parent;

-- RLS policy WITH CHECK expressions follow the same revalidation path.
CREATE TABLE policy_check_kj_parent (id int PRIMARY KEY);
CREATE TABLE policy_check_kj_child
(
    id int PRIMARY KEY,
    parent_id int NOT NULL REFERENCES policy_check_kj_parent (id)
);
CREATE TABLE policy_check_kj_guarded (id int);
INSERT INTO policy_check_kj_parent VALUES (1);
INSERT INTO policy_check_kj_child VALUES (10, 1);
ALTER TABLE policy_check_kj_guarded ENABLE ROW LEVEL SECURITY;
CREATE POLICY policy_check_kj ON policy_check_kj_guarded
FOR INSERT
WITH CHECK (EXISTS (
    SELECT 1
    FROM policy_check_kj_parent p
    -- accepted
    JOIN policy_check_kj_child c FOR KEY (parent_id) -> p (id)
));

-- rejected, reason: stored policy WITH CHECK expression depends on the FK proof
ALTER TABLE policy_check_kj_child
    ALTER CONSTRAINT policy_check_kj_child_parent_id_fkey NOT ENFORCED;

-- After dropping the policy, the same DDL succeeds.
DROP POLICY policy_check_kj ON policy_check_kj_guarded;
ALTER TABLE policy_check_kj_child
    ALTER CONSTRAINT policy_check_kj_child_parent_id_fkey NOT ENFORCED;
SELECT *
FROM policy_check_kj_parent p
-- rejected, reason: FK is now not enforced
JOIN policy_check_kj_child c FOR KEY (parent_id) -> p (id);
DROP TABLE policy_check_kj_guarded,
           policy_check_kj_child,
           policy_check_kj_parent;
