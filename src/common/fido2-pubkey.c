 ...
 * and extracts the EC point and credential information needed for FIDO2
 * authentication.
 *
 ...
 *
 * On success, fills in the fido2_parsed_pubkey structure and returns true.
 * On failure, sets *errmsg and returns false.
 *
 * The caller must free the allocated fields in the structure when done.
 */
bool
fido2_parse_openssh_pubkey(const char *pubkey_str,
						   ...
						   char **errmsg)
{
	...
	*errmsg = NULL;
	memset(result, 0, sizeof(Fido2ParsedPubkey));
	...
	result->algorithm = FIDO2_ALG_ES256;
 ...
 */
void
fido2_free_parsed_pubkey(Fido2ParsedPubkey *pubkey)
{
	if (pubkey)
	{
