 ...
 *
 *	  This catalog stores FIDO2/WebAuthn public keys for role authentication.
 *
...
CATALOG(pg_role_pubkeys,6500,RolePubkeysRelationId) BKI_SHARED_RELATION BKI_ROWTYPE_OID(6501,RolePubkeysRelation_Rowtype_Id) BKI_SCHEMA_MACRO
{
	...
	int64		sign_count;		/* last signature counter (anti-replay) */

	/*
	 * Variable-length fields; must use heap_getattr() to access.
	 */
#ifdef CATALOG_VARLEN
	bytea		credential_id BKI_FORCE_NOT_NULL;	/* authenticator-assigned credential ID */
